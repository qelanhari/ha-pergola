...
- **`coordinator.py` (`PergolaCoordinator`)** — the brain. Owns the periodic control loop, persisted state (via `homeassistant.helpers.storage.Store`), state-change subscriptions for sun/PV/humidity/rain/lock entities, morning calibration sequence, and cloud-state hysteresis. State-change listeners only *request a refresh*; they never act directly, and they skip the refresh when `old_state is None` (an entity being added at startup — refreshing there once commanded a movement ~1 s after boot, before the cover integration was ready). All entities (`sensor`, `binary_sensor`, `select`, `button`) are thin views over `coordinator.data`.
...
3. Applies overrides in priority order:
   ...
   - **Safety lock** — only `LOCK_CLOSING_ORIGINS` (`temperature`/`security`) act: close idempotently (skipped when already < 5%) and hold. A `rain` origin is **ignored** — the rain sensor above is the authority and this sensor lags it by many minutes and can stick, so honouring it could wedge the pergola. Any reported origin does, however, mark a failed movement as *refused* rather than a fault (`_async_move_and_verify` leaves `_consecutive_failures` and `_last_known_position` untouched).
